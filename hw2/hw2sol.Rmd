...
# bar chart?
by_ID %>%
  filter(LANGUAGE != NaN) %>%  # c = filter(by_ID, LANGUAGE!=NaN) worked, c = filter(by_ID, !is.nan(by_ID$LANGUAGE)) didn't work
  ggplot() + 
...
Death:
```{r}
# filter and add new variable, is.nan(death time) => healthy;  ! is.nan(death time) => dead?
# by_ID %>%
#   ggplot() + 
#   geom_bar(mapping=aes(stringr::str_wrap(ETHNICITY,10), y = ..prop.., group = 1))+
#   labs(x='Maritial Status', y='Percentage')+

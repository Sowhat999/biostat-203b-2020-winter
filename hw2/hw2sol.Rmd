...
For this hospital, a major group of the "patients" are newborn baby (around 14%, more than 8000). It did not take few patients between 1~16 years old, and its patients' age range from 15~90. It seems it took more senior patients than youger ones.
```{r}
f<- JointT %>%
...
Most patients stayed in ICU less than 10 days, but some patients stayed in ICU for as long as 175 days.
```{r warning=FALSE}
f<- Icustay %>%
...
Same as the Gender distribution. The age distribution for ICU admission is similar to the general hospital admission. For this hospital, a major group of the "patients" are newborn baby (around 14%, more than 8000). It did not take few patients between 1~16 years old, and its patients' age range from 15~90. It seems it took more senior patients than youger ones.
```{r}
...
7. Distinct the above result tibble by `SUBJECT_ID` since multiple systolic measurement could be done for them during their first ICU stay.

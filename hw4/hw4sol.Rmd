...
`inner_join` the table of admission `admissions` with the table of influenza-caused admission to pull the admitting and discharge time. Here the `hospital_expire_flag` is a logical variable indicating whether the patient died (1) in the hospital or not (0). 

Also join the `patients` table to get more detailed demographic information of the patients admitted into hospital. Here we want information about gender, date of birth, date of death and expire_flag.
```{r}
...
Calculate the age for all patients in the table. According to the documentation for the patients table, the patients of 90 years and older had their ages artficially inflated, and thus removed from the table.

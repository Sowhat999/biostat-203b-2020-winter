...
```{r, eval = FALSE}
# they don't have data on goolge sheet anymore
...
confirmed_long <- confirmed %>%
  pivot_longer(-(`Province/State`:Long), 
               names_to = "time", 
               values_to = "confirmed") %>%
  mutate(time = (mdy_hm(time))) # convert string to date-time (need to change it to datetime type)
confirmed_long
```
```{r}
